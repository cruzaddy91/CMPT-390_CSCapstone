...
const COLUMN_META = [
  ...
  { key: 'percent_1rm', label: '% 1RM', width: 8, hint: 'Intensity as a percentage of one-rep max. Example: 75%, 80, 82.5%.' },
  { key: 'rpe', label: 'RPE', width: 6, hint: 'Rate of perceived exertion (1-10). Use instead of or alongside %.' },
  { key: 'weight', label: 'Weight', width: 10, hint: 'Absolute load in kg or lb. Example: 100kg, 225lb.' },
  { key: 'tempo', label: 'Tempo', width: 10, hint: 'Eccentric-pause-concentric-top, e.g. 3-1-X-1. Leave blank if not prescribed.' },
  { key: 'rest', label: 'Rest', width: 8, hint: 'Inter-set rest. Example: 2min, 90s.' },
...
const buildInstructionsSheet = () => {
  const rows = [
    ...
    ['Reps like "1+1+1" or "2+1" represent a single-rep complex, e.g. a snatch + overhead'],
